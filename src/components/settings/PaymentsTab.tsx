...
import { 
  ...
  Calendar, Filter, PieChart, BarChart3, Sparkles, ChevronRight,
  FileText, Building2, Smartphone, Globe, Search, X
} from 'lucide-react';
...
// Mock data for demo
const mockFees: Fee[] = [
  { id: '1', amount: 2500000, paid_amount: 2500000, due_date: '2025-01-15', semester: 'Semester 1', academic_year: '2024/2025', description: 'Tuition Fees' },
  { id: '2', amount: 500000, paid_amount: 300000, due_date: '2025-02-01', semester: 'Semester 1', academic_year: '2024/2025', description: 'Functional Fees' },
  { id: '3', amount: 150000, paid_amount: 0, due_date: '2025-03-01', semester: 'Semester 1', academic_year: '2024/2025', description: 'Library Fees' },
  { id: '4', amount: 1200000, paid_amount: 1200000, due_date: '2024-08-15', semester: 'Semester 2', academic_year: '2023/2024', description: 'Tuition Fees' },
];

const mockPayments: Payment[] = [
  { id: '1', amount: 2500000, paid_at: '2025-01-10', payment_method: 'Mobile Money', status: 'completed', transaction_ref: 'TXN-001-2025' },
  { id: '2', amount: 1200000, paid_at: '2024-08-10', payment_method: 'Bank Transfer', status: 'completed', transaction_ref: 'TXN-087-2024' },
  { id: '3', amount: 300000, paid_at: '2025-01-20', payment_method: 'Mobile Money', status: 'completed', transaction_ref: 'TXN-015-2025' },
];
  ...
  const fetchData = async () => {
    try {
      ...
      const [feesRes, paymentsRes] = await Promise.all([
        supabase.from('fees').select('*').eq('student_id', user?.id).order('due_date', { ascending: false }),
        supabase.from('payments').select('*').eq('student_id', user?.id).order('paid_at', { ascending: false })
      ]);

      if (feesRes.data?.length) setFees(feesRes.data);
  ...
  return (
    <div className="relative">
      ...
      <div className="relative space-y-8">
        ...
        >
          {[
            ...
            { 
              ...
              gradient: outstanding > 0 ? 'from-amber-500 to-orange-500' : 'from-emerald-500 to-teal-500',
              trend: null,
              prefix: 'UGX '
            },
            { 
            ...
            >
              <Card className="relative overflow-hidden border-0 shadow-xl bg-card/80 backdrop-blur-sm h-full">
                ...
                <div className={`absolute top-0 right-0 w-24 h-24 bg-gradient-to-bl ${stat.gradient} opacity-10 rounded-bl-full`} />
                ...
                <CardContent className="pt-6 relative">
                  <div className="flex items-start justify-between mb-4">
                    ...
                    {stat.trend && (
                      <Badge variant="secondary" className="gap-1 bg-emerald-500/10 text-emerald-600 border-0">
              ...
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    ...
                    <CardDescription className="text-base">Your fee payment completion status</CardDescription>
                  </div>
                </div>
                <div className="flex items-center gap-4">
                  <motion.div 
            ...
            <CardContent className="relative pt-2">
              ...
              <div className="grid sm:grid-cols-3 gap-6">
                <div className="p-4 rounded-2xl bg-emerald-500/5 border border-emerald-500/20">
                  <p className="text-xs font-medium text-muted-foreground uppercase tracking-wider mb-1">Amount Paid</p>
                ...
                <div className="p-4 rounded-2xl bg-amber-500/5 border border-amber-500/20">
                  ...
                  <p className="text-xl font-bold text-amber-600">UGX {outstanding.toLocaleString()}</p>
                </div>
                <div className="p-4 rounded-2xl bg-primary/5 border border-primary/20">
                  <p className="text-xs font-medium text-muted-foreground uppercase tracking-wider mb-1">Total Fees</p>
                  <p className="text-xl font-bold text-primary">UGX {totalFees.toLocaleString()}</p>
                </div>
              </div>
            </CardContent>
          </Card>
                      ...
                      >
                        ...
                        <div 
                          className={`absolute inset-0 rounded-2xl opacity-20 ${
                            isPaid ? 'bg-gradient-to-r from-emerald-500/30 to-transparent' :
                            isPartial ? 'bg-gradient-to-r from-amber-500/30 to-transparent' :
                        ...
                        <div className="relative">
                          <div className="flex items-start justify-between mb-3">
                            <div>
                              <p className="font-bold text-lg">{fee.description}</p>
                            ...
                            <Badge className={`${
                              ...
                              isPartial ? 'bg-amber-500/10 text-amber-600 border-amber-500/30' :
                          ...
                          <div className="flex items-end justify-between">
                            <div>
                              ...
                              {isPartial && (
                                <p className="text-sm">
                                  <span className="text-emerald-600 font-semibold">UGX {fee.paid_amount.toLocaleString()}</span>
          ...
          >
            <Card className="border-0 shadow-xl bg-card/80 backdrop-blur-sm h-full">
              ...
              <CardContent>
                    ...
                    return (
                      ...
                      >
                        <div className="flex items-center gap-4">
                          ...
                          <div>
                            <p className="font-bold text-lg">UGX {payment.amount.toLocaleString()}</p>
                            <p className="text-sm text-muted-foreground">{payment.payment_method}</p>
                            <p className="text-xs text-muted-foreground font-mono">{payment.transaction_ref}</p>
                          </div>
                        </div>
                ...
                {/* View All Button */}
                <Button variant="outline" className="w-full mt-6 h-12 rounded-xl gap-2 border-dashed">
              ...
              >
                <Card className="border-0 shadow-2xl overflow-hidden">
                  ...
                  <CardContent className="p-6 space-y-4">
                    {[
                      { label: 'Transaction Ref', value: selectedPayment.transaction_ref },
                      { label: 'Payment Method', value: selectedPayment.payment_method },
                      { label: 'Status', value: selectedPayment.status },
                      { label: 'Date', value: new Date(selectedPayment.paid_at).toLocaleString() },
                    ].map((item) => (
        ...
        >
          {[
            { title: 'Generate PRN', desc: 'Create payment reference', icon: Sparkles, gradient: 'from-primary to-primary/70' },
            { title: 'Payment Statement', desc: 'Download full history', icon: FileText, gradient: 'from-secondary to-secondary/70' },
            { title: 'Contact Finance', desc: 'Get support', icon: Building2, gradient: 'from-accent to-accent/70' },
          ].map((action, i) => (
            <motion.div
              key={action.title}
            ...
            >
              <Card className="border-0 shadow-lg cursor-pointer overflow-hidden group">
                <CardContent className="p-6 flex items-center gap-4">
                  <div className={`h-12 w-12 rounded-2xl bg-gradient-to-br ${action.gradient} flex items-center justify-center shadow-lg group-hover:scale-110 transition-transform`}>
