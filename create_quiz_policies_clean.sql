...
CREATE POLICY "Lecturers can view own quizzes" ON quizzes
FOR SELECT USING (lecturer_id = auth.uid());

CREATE POLICY "Lecturers can create quizzes" ON quizzes
FOR INSERT WITH CHECK (lecturer_id = auth.uid());

CREATE POLICY "Lecturers can update own quizzes" ON quizzes
FOR UPDATE USING (lecturer_id = auth.uid())
WITH CHECK (lecturer_id = auth.uid());

CREATE POLICY "Lecturers can delete own quizzes" ON quizzes
